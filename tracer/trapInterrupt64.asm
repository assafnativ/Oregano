...
trap_interrupt_in_range:
    ...
    mov BYTE [rax], 0
    ; We logged the last executed opcode, so we need the last EIP
    inc rax
    ; Write EIP
    mov QWORD [rax], rbx
    ; pos += sizeof( EIP )
...
; TODO: On x64 we should use one huge cyclc buffer. It will be faster.
trap_interrupt_check_log_buffer:
    ...
    jnz LOG_BUFFER_HAS_SPACE
        ...
        mov rax, [rax + rdx * SIZE_OF_POINTER]
...
CLEAR_TRAP_FLAG_AND_RET:
    ...
    mov DWORD [rdi], eax
    ; Clear the trap flag
    mov rax, QWORD [rsp + SAVED_RFLAGS]
...
trap_interrupt_out_of_range:
    ; We got RIP in ebx at this point
    lea rcx, QWORD [loggingRanges]
    cmp [rcx], rbx
    jbe BOTTOM_RANGE_FOUND
    ; We assume that the ranges are sorted by BOTTOM
    ...
    NOT_IN_ANY_RANGE:
    ; When we return to logging, we will have to skip one log cycle
    ; so all registers would get the right values
    ; We have to do it both here and in the range found, because it might
    ; get back right into the current logging range
    mov DWORD [rbp + SKIP_MEM_LOG], 1
    ; Return for now
    ; Note that we didn't write the last writting offset to the log,
    ; because we want the trace to overwrite this cycle
    RETURN_FROM_INTERRUPT_WITH_CHECK
